...
<% if(request.getParameter("BADA_ID") != null){ %>
	...
	<input type="hidden" name="PERS_ID" id="PERS_ID" value="<%= (String)request.getSession().getAttribute("PERS_ID")%>" >
		<h1>Editar la Base de datos</h1>
			<p>Gestione la información de la Base de Datos.</p>

			<%
			out.print(new baseDatos().editar( request.getParameter("BADA_ID")));
			%>
...
<% if(request.getParameter("accion") != null && request.getParameter("accion").equals("guardar")){ %>
	...
	<% out.print(new baseDatos().update(request)); %>
	
	<div id="stylized" class="myform">
		<h1>Detalle de la base de datos</h1>
		<p>Información detallada de la base de datos.</p>
