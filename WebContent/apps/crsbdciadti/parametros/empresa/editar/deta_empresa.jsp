...
<% if(request.getParameter("EMPR_ID") == null){ %>
...
<p>Diligencie el siguiente formulario para editar la empresa seleccionada.</p>
			
			<%
			out.print(new empresa().editar( request.getParameter("EMPR_ID")));
			%>
...
<% if(request.getParameter("accion") != null && request.getParameter("accion").equals("guardar")){ %>
		...
		<% out.print(new empresa().update( request )); %>
	...
	<div id="stylized" class="myform">
		...
		<%
		out.print(new empresa().visualizar( request.getParameter("EMPR_ID") ));
		%>
		<div class="spacer"></div>
	</div>
